...
class _HekaStoreAppContent extends StatelessWidget {
  const _HekaStoreAppContent({
    required this.themeState,
    required this.langState,
  });

  final dynamic themeState;
  final dynamic langState;
      ...
      listeners: [
        ...
        BlocListener<CartBloc, CartState>(
          ...
          listener: (_, state) {
            if (state.addedToCartSuccess == true) {
              ...
              _showSnackBar(
                message: state.error?.serverMessage ?? t.error_unknown_message,
                isSuccess: false,
              );
            }
          },
        ...
        BlocListener<WishlistBloc, WishlistState>(
          ...
          listener: (_, state) {
            if (state.error != null) {
              _showSnackBar(
                message: state.error?.serverMessage ?? t.something_went_wrong,
                isSuccess: false,
              );
            } else {
              final isAdded = state.isInWishlist(state.lastToggledProductId!);
              _showSnackBar(
                message: isAdded ? t.addedToWishlist : t.removedFromWishlist,
                icon: isAdded
